...
static CGFloat scaleAwareFloor(CGFloat f) {
    CGFloat scale = [UIScreen mainScreen].scale;
    return floor(f*scale)/scale;
}

CGRect CGRectPixelAligned(CGRect rect) {
    return CGRectMake(scaleAwareFloor(rect.origin.x), scaleAwareFloor(rect.origin.y), rect.size.width, rect.size.height);
}
...
CGRect CGRectWithEdgeInsets(CGRect rect, UIEdgeInsets edgeInsets)
{
    if (rect.size.width - edgeInsets.right - edgeInsets.left < 0 || rect.size.height - edgeInsets.bottom - edgeInsets.top < 0)
    {
        return CGRectZero;
    }
    
    return CGRectMake(rect.origin.x + edgeInsets.left, rect.origin.y + edgeInsets.top, rect.size.width - edgeInsets.right - edgeInsets.left, rect.size.height - edgeInsets.bottom - edgeInsets.top);
}